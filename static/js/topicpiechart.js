...
function bindPieChartData(topic_id){
    $.ajax({
    type: "GET",
    url: "getPieChartData",
    dataType: 'json',
    ...
    success: function (data) {
        ...
        for (var i = 0; i < data.length; i++){
            temp_sum += parseFloat(data[i][1]);
        }
        
        var peichartdata = [];
        for (var i = 0; i < data.length; i++){
            peichartdata.push({name: data[i][0], percent: parseFloat(data[i][1]/temp_sum*100).toFixed(2)})
        }
...
function darwPieChart(pieData){
    ...
    var pie_width = 380;
    var pie_height = 340;
    var width = pie_width - margin.left - margin.right, //200是legend的长度
        height = pie_height + legend_width - margin.top - margin.bottom;
...
//        .attr("fill", "white");
	...
	legend.selectAll('rect')
        ...
        .enter()
        .append("rect")
        .attr("x", function(d, i){ return extra_width + i * 30 - width/2 + 3 ;} )
	...
	legend.selectAll('text')
        ...
        .enter()
        .append("text")
        .attr("x", function(d, i){ return extra_width + i * 30 - width/2;})
